...
function HistoryCard({ requestNumber, date, jsonData }: HistoryCardProps) {
  ...
  return (
    <div className="flex items-center justify-between space-x-4  w-full px-3 py-2 rounded-2xl border-2 border-orange-800">
      <div className="space-y-1 flex items-center justify-center">
        <div className="w-14">
          <div className="relative w-4 h-4 rounded-full border border-gray-200 bg-white dark:border-orange-700 dark:bg-orange-800" />
        </div>
      ...
      <div className="flex items-center justify-end gap-2 flex-col sm:flex-row">
        <BudgetHistoryButton xlsxName={requestNumber}>
          <Button
            variant="bordered"
            className="border-orange-800 w-full text-sm"
          >
            Presupuesto
          </Button>
        </BudgetHistoryButton>
